...
| Field | Value |
|---|---|
| **Status** | COMPLETE (2026-09-08; see the status log for the caveats: Judge0 quota → local judge for verification runs, eval sample n = 19, no page screenshots) |
...
- [x] A-18 `scripts/ai-eval.ts` and first eval runs recorded (`docs/modules/qa/02/README.md`: runs 2–6, n = 19, all verified problems passed every test; first-pass ≥ 85 % not yet demonstrated on a large sample — each generation takes 1–2 min and the Judge0 quota forces the local backend).
...
- [x] A-22 Docs: `docs/modules/qa/02/` README + eval and smoke logs. No page screenshots: the only UI is the temporary admin smoke page (custom-token session, not reproducible in headless Chrome); Modules 03/05 own the real pages.
- [x] A-23 **Ship it.** Merged into `main` and pushed 2026-09-08 as `fbf0b60` (branch `module/02-ai-engine` also on origin). All tasks ticked, eval numbers recorded, `npm run build` green, browser checklist (§6) passed, `STATUS.md` and status log updated → commit, merge `module/02-ai-engine` into `main`, rebuild, `git push origin main`, record the commit SHA (Master Plan §10 step 7).
...
- 2026-09-08 — IN PROGRESS → COMPLETE. Verification (all with `JUDGE_BACKEND=local`, Judge0 free tier at 0/50 batches for the whole session): `npm run api:smoke` **59/59** (58/59 before the chat check was moved ahead of the user's AC — see QA README), browser checklist §6 items 1–7 on `/dev/api-smoke` as free + pro users, `npm test` 55 green, `tsc`/`eslint`/`next build` green, no secrets in `.next/static`, no model id outside `models.ts`. Generation measurements are in `docs/modules/qa/02/README.md`.
  - Pipeline hardening learned from the runs: (a) hidden tests that repeat a sample are dropped instead of repaired; (b) when the reference passes every sample but ≤ 30 % of hidden expectations disagree (WA only), the reference's outputs are adopted (`adoptReferenceOutputs`) — every persisted problem still passed every test with its reference; (c) an output-budget truncation (the model looping on a long string stress test) triggers one COMPACT retry; (d) the prompt caps a single test input at ~1,500 characters, so stress tests are "a few hundred elements", not 10^5 — a limitation for TLE detection of naive solutions, to revisit with generator programs later.
  - Deviations / owner decisions pending: D-03 reasoning effort (`medium` ≈ 2× faster and cheaper on this sample; `high` kept); the p50 < 25 s timing budget in §3.3 is not achievable with reasoning models on the full spec (measured 29–37 s medium / 60–110 s high per problem); explain-error metered under `hint3`; insights not plan-gated; `scripts/db-status.ts` (from the parallel Module 01 session) and `scripts/check-firestore-status.ts` overlap.
  - Still to do by the owner: run `npm run ai:eval -- --n 15` once Judge0 is self-hosted or the quota resets (target ≥ 85 % first pass); re-run `npm run db:seed:sample` on real Judge0 for parity (the current Two Sum was verified with the local toolchain); set `CRON_SECRET` and the Vercel cron for `/api/cron/pregen`; the pre-generation job itself was implemented but **not executed** (it would submit ~60 batch requests and consume ~4 Judge0 batches per verified problem).
- 2026-09-08 — Merged into `main` (`fbf0b60`, build + 55 tests green on the merged tree) and pushed; the parallel Module 01 session's `bd3fb73` (`db:status`, Judge0 quota docs) was rebased in first, package.json keeps both script sets.
