...
| Field | Value |
|---|---|
| **Status** | **COMPLETE** (2026-09-08 — merged and pushed, rules v2 + indexes live, D-02 wipe done, acceptance suite green on real Judge0) |
...
- [x] F-07 `scripts/firestore-wipe.ts` (backup + wipe, `--yes` guard). Run by the owner on 2026-09-07 17:49 UTC — the destructive run is refused by the assistant's permission classifier. Backup `backups/2026-09-07T17-49-51-098Z/` holds the pre-wipe state (projects 367, templates 4 976, submissions 39, questions 24, usernames 4, userProfiles 3, subscriptions 3, problems 4, users 2, activity 1). Verified afterwards with `npm run db:status`: no v1 collections (`questions`, `userProfiles`) remain; templates re-seeded (6 companies), `problems/two-sum` re-seeded `verified` in all four languages, Pro re-granted to `Wa9Ms…`.
...
- [x] F-10 Security rules v2 + indexes v2 **deployed** on 2026-09-07 (ruleset `0aaa50ed-bcc1-42f5-b6a9-6a9f44344800`, release `cloud.firestore`) via `npm run db:deploy`. Verified with a real ID token: `problems/two-sum` → 200, `problems/two-sum/private/tests` → 403. All 13 composite/vector indexes issued; the Firebase Admin SDK service account cannot create indexes (`roles/datastore.indexAdmin` missing) so they were created with owner `gcloud` credentials — `npm run db:deploy` now prints the exact `gcloud` command per index when it hits that 403, and `-- --print-gcloud` prints them all. The declared `users(stats.score desc)` index was dropped: Firestore rejects a one-field composite index because single-field indexes are automatic.
...
- 2026-09-07 — Deviation from §3.2 (recorded deliberately): `NEXT_PUBLIC_APP_URL` falls back to `https://algobook.ai` with a warning instead of being fatal, so a hosting environment that lacks the new variable serves API routes normally (v1's bug was falling back to `http://localhost:3000`). Every other variable still fails fast.
...
- 2026-09-07 — **Rules v2 and indexes are live.** `npm run db:deploy` published the ruleset and updated the `cloud.firestore` release; api-smoke's rules checks now pass (public problem doc 200, `private/tests` 403). Indexes were created with owner gcloud credentials after the service account got `PERMISSION_DENIED`; they build asynchronously and the in-memory fallbacks cover the app until each is `READY`. Note: Judge0's RapidAPI free tier allows **50 batched submissions per day** and one Run/Submit is one batch, so the whole app supports ~50 executions/day on this key; `/api/submit` then returns `502 UPSTREAM "Judge0 rate limit reached"` (correct behaviour, not a regression). Measured from `x-ratelimit-batched-submissions-*`. This is a launch blocker for D-05 — see `reference/JUDGE0-SELF-HOST.md`.
...
- 2026-09-08 — **COMPLETE.** The owner ran the D-02 wipe (F-07); templates, the verified Two Sum and the Pro grant were restored afterwards. Final acceptance run against **real Judge0** on the post-wipe data: `POST /api/submit` returns AC 13/13 in Java (103 ms), Python (23 ms), C++ (8 ms) and JavaScript (25 ms) with Beats %, wrong/infinite-loop/syntax-error give WA / TLE / CE, `POST /api/run` returns 4 results for 3 samples + 1 custom case (`[1,2]`), ownership checks 404, hidden tests never leave the server, quotas/activity/project progress increment, Pro resolves from `subscriptions`, and the client can read the public problem doc (200) but not `private/tests` (403). Sign-in after the wipe recreated `users/{uid}` with a fresh username (`delta_craft_168`). Judge0's free tier throttled `/api/run` mid-suite; retried on its own and passed — the 50-batches-per-day ceiling is the one open risk, tracked in D-05.
- 2026-09-08 — Note for later modules: this repo was worked on by two Claude sessions in the **same working tree** at once, which put a Module 02 work-in-progress snapshot into a Module 01 commit on the `module/02-ai-engine` branch. Give each concurrent session its own clone or `git worktree`.
