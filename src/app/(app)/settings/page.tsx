    ...
    <div className="grid gap-6 lg:grid-cols-[200px_1fr]">
      ...
      <div className="space-y-4">
        ...
        <Section id="editor" title="Editor defaults" description="Mirrors the workspace Settings dialog; saved to your account.">
          <Row label="Font"><Select value={editor.font} onValueChange={(v) => setEditor({ font: v })}><SelectTrigger className="w-44"><SelectValue /></SelectTrigger><SelectContent>{FONT_OPTIONS.map((f) => <SelectItem key={f} value={f}>{f}</SelectItem>)}</SelectContent></Select></Row>
          <Row label="Font size"><Select value={String(editor.fontSize)} onValueChange={(v) => setEditor({ fontSize: Number(v) })}><SelectTrigger className="w-24"><SelectValue /></SelectTrigger><SelectContent>{[12, 13, 14, 15, 16, 18, 20].map((n) => <SelectItem key={n} value={String(n)}>{n}px</SelectItem>)}</SelectContent></Select></Row>
          <Row label="Tab size"><Select value={String(editor.tabSize)} onValueChange={(v) => setEditor({ tabSize: Number(v) })}><SelectTrigger className="w-24"><SelectValue /></SelectTrigger><SelectContent>{[2, 4, 8].map((n) => <SelectItem key={n} value={String(n)}>{n}</SelectItem>)}</SelectContent></Select></Row>
          <Row label="Key bindings"><Select value={editor.keyBinding} onValueChange={(v) => setEditor({ keyBinding: v as typeof editor.keyBinding })}><SelectTrigger className="w-32"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="standard">Standard</SelectItem><SelectItem value="vim">Vim</SelectItem><SelectItem value="emacs">Emacs</SelectItem></SelectContent></Select></Row>
          <Row label="Default language"><Select value={editor.language} onValueChange={(v) => setEditor({ language: v as typeof editor.language })}><SelectTrigger className="w-36"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="java">Java</SelectItem><SelectItem value="python">Python 3</SelectItem><SelectItem value="cpp">C++</SelectItem><SelectItem value="javascript">JavaScript</SelectItem></SelectContent></Select></Row>
